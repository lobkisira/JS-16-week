...
function makeTwo() {
  ...
  paragraphTwo.innerHTML = form.getAttribute("name");
}
...
function makeThree() {
  ...
  paragraphThree.innerHTML = form.getAttribute("name");
}
...
function makeFive() {
  ...
  paragraphFive.innerHTML = form.elements.length;
}
...
function makeSix() {
  ...
  paragraphSix.innerHTML = form.elements.length;
}
...
function makeSeven() {
  ...
  for (let i of formElements) {
    elementsList.push(i.getAttribute("name"));
    i++;
  }
...
function makeEight() {
  const form = document.forms[0];
  const forms = form.elements;
  for (let i of forms) {
    formFirstNames.push(i.getAttribute("name"));
  }
  paragraphEight.innerHTML = formFirstNames.join(", ");
}
...
function makeNine() {
  ...
  const forms = form.elements;
  for (let i of forms) {
    formThirdNames.push(i.getAttribute("name"));
  }
  paragraphNine.innerHTML = formThirdNames.join(", ");
}
...
let formForthRadioValue = "";

function makeTen() {
  const form = document.forms.lastForm;
  const radio = form.elements.fourthName;
  formForthRadioValue += radio.value;
  paragraphTen.innerHTML = formForthRadioValue;
}
...
function makeEleven() {
  ...
  const radio = form.elements.firstSelect;
  Array.from(radio).forEach((element) => {
    optionsValues += element.value;
...
function checkButton(e) {
  e.preventDefault();
  const form = document.forms.lastForm;
  const check = form.elements.fourthName;
  if (check.checked) {
...
function makeSixteen() {
  const form = document.forms.formTwo;
  const select = form.elements.checkboxThree;
  select.checked = true;
}
...
formOne.addEventListener("submit", function (event) {
  ...
  const inputName = formOne.elements.firstName;
  const inputEmail = formOne.elements.firstEmail;
  if (inputName.value === "" && inputEmail.value === "") {
  }
  paragraphSeventeen.textContent = "Заполните все поля";
});
...
const formFirst = document.forms.formOne;
const selectElement = formFirst.elements.firstSelect;
...
emailInput.oninput = function () {
  ...
  if (emailInput.value.match(regexp)) {
    ...
    paragraphSeventeen.textContent =
      "Неправильно введен формат Электронной почты";
  }
};
...
document.querySelector(".b-21").onclick = function (event) {
  ...
  if (!isChecked) {
    ...
    document.getElementById("result21").textContent =
      "Ошибка! Ни одно значение не выбрано.";
    //Ваш код
  } else {
...
document.querySelector(".b-22").onclick = function (event) {
  ...
  if (nameInput.value.trim() === "") {
    ...
    document.getElementById("result22").textContent =
      "Ошибка! Поле имя не заполнено."; //Ваш код
  } else {
...
form.addEventListener("submit", function (event) {
  event.preventDefault();
  const input = form.elements.fourthName;
...
document.querySelector(".b-24").onclick = function () {
  const allElm = document.querySelectorAll("placeholder");
  allElm.forEach(function (element) {
    element.value.setAttribute("Введите данные");
  });
};
...
document.querySelector(".b-25").onclick = function () {
  ...
  inputs.forEach(function (input) {
    input.addEventListener("focus", function () {
      document.body.style.border = "1px solid red"; //Ваш код
    });

    input.addEventListener("blur", function () {
      return input; //Ваш код
    });
  });
};
...
const formThirdPlaceholder = document.forms.formThree;
const placeholderValue = formThirdPlaceholder.elements.thirdName;
const result26 = document.getElementById("result26");
document.querySelector(".b-26").onclick = function (event) {
  event.preventDefault();
  result26.innerHTML = placeholderValue.getAttribute("placeholder");
};
...
formTwoInputs.forEach(function (input) {
  input.addEventListener("input", function () {
    result27.innerHTML = "Изменение внесено";
  });
});
...
selectFormThree.addEventListener("change", function () {
  result28.innerHTML = "Опция выбрана";
});
...
function handleSubmitTwentyNine(event) {
  ...
  if (nameInputThree.value && emailInputThree.value) {
    console.log(nameInputThree.value, emailInputThree.value); //Ваш код
  } else {
...
function handleSubmit(event) {
  ...
  console.log(selectedOptions); //Ваш код
}
